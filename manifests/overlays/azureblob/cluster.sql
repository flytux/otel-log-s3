...
DROP NAMED COLLECTION IF EXISTS azure_otel_logs;
DROP NAMED COLLECTION IF EXISTS azure_hyperdx_sessions;
DROP NAMED COLLECTION IF EXISTS azure_otel_traces;
DROP NAMED COLLECTION IF EXISTS azure_otel_metrics;

CREATE NAMED COLLECTION azure_otel_logs AS
    storage_account_url = '__AZURE_STORAGE_ACCOUNT_URL__',
    container = 'otel-logs',
    blob_path = 'logs/year=*/month=*/day=*/hour=*/minute=*/*.json',
    account_name = '__AZURE_STORAGE_ACCOUNT_NAME__',
    account_key = '',
    format = 'JSONAsString',
    compression = 'none',
...
CREATE NAMED COLLECTION azure_hyperdx_sessions AS
    storage_account_url = '__AZURE_STORAGE_ACCOUNT_URL__',
    container = 'hyperdx-sessions',
    blob_path = 'sessions/year=*/month=*/day=*/hour=*/minute=*/*.json',
    account_name = '__AZURE_STORAGE_ACCOUNT_NAME__',
    account_key = '',
    format = 'JSONAsString',
    compression = 'none',
...
CREATE NAMED COLLECTION azure_otel_traces AS
    storage_account_url = '__AZURE_STORAGE_ACCOUNT_URL__',
    container = 'otel-traces',
    blob_path = 'traces/year=*/month=*/day=*/hour=*/minute=*/*.json',
    account_name = '__AZURE_STORAGE_ACCOUNT_NAME__',
    account_key = '',
    format = 'JSONAsString',
    compression = 'none',
...
CREATE NAMED COLLECTION azure_otel_metrics AS
    storage_account_url = '__AZURE_STORAGE_ACCOUNT_URL__',
    container = 'otel-metrics',
    blob_path = 'metrics/year=*/month=*/day=*/hour=*/minute=*/*.json',
    account_name = '__AZURE_STORAGE_ACCOUNT_NAME__',
    account_key = '',
    format = 'JSONAsString',
    compression = 'none',
...
ENGINE = Distributed('otel_cluster', 'default', 'otel_logs_local', cityHash64(TraceId, SpanId, ServiceName));

CREATE TABLE IF NOT EXISTS hyperdx_sessions_local_dist AS hyperdx_sessions_local
ENGINE = Distributed('otel_cluster', 'default', 'hyperdx_sessions_local', cityHash64(TraceId, SpanId, ServiceName));

CREATE TABLE IF NOT EXISTS otel_traces_local_dist AS otel_traces_local
ENGINE = Distributed('otel_cluster', 'default', 'otel_traces_local', cityHash64(TraceId, SpanId, ServiceName));

CREATE TABLE IF NOT EXISTS otel_metrics_gauge_local_dist AS otel_metrics_gauge_local
ENGINE = Distributed('otel_cluster', 'default', 'otel_metrics_gauge_local', cityHash64(MetricName, ServiceName));

CREATE TABLE IF NOT EXISTS otel_metrics_sum_local_dist AS otel_metrics_sum_local
ENGINE = Distributed('otel_cluster', 'default', 'otel_metrics_sum_local', cityHash64(MetricName, ServiceName));

CREATE TABLE IF NOT EXISTS otel_metrics_histogram_local_dist AS otel_metrics_histogram_local
ENGINE = Distributed('otel_cluster', 'default', 'otel_metrics_histogram_local', cityHash64(MetricName, ServiceName));

CREATE TABLE IF NOT EXISTS otel_metrics_exp_histogram_local_dist AS otel_metrics_exp_histogram_local
ENGINE = Distributed('otel_cluster', 'default', 'otel_metrics_exp_histogram_local', cityHash64(MetricName, ServiceName));
...
FROM (
    ...
    FROM (
        ...
        FROM azureBlobStorage(azure_otel_logs)
    )
...
FROM (
    ...
    FROM (
        SELECT JSONExtractArrayRaw(json, 'resourceLogs') AS resource_logs
        FROM azureBlobStorage(azure_hyperdx_sessions)
    )
    ARRAY JOIN resource_logs AS rl
    ARRAY JOIN JSONExtractArrayRaw(rl, 'scopeLogs') AS sl
    ARRAY JOIN JSONExtractArrayRaw(sl, 'logRecords') AS lr
...
FROM (
    ...
    FROM (
        ...
        FROM azureBlobStorage(azure_otel_traces)
    )
...
FROM (
    ...
    FROM (
        ...
        FROM azureBlobStorage(azure_otel_metrics)
    )
